...
type Pather interface {
	...
	// can be pathed to.
	PathNeighbors(sessionId int64) []Pather
	// PathNeighborCost calculates the exact movement cost to neighbor nodes.
	PathNeighborCost(sessionId int64, to Pather) float64
	// PathEstimatedCost is a heuristic method for estimating movement costs
	// between non-adjacent nodes.
	PathEstimatedCost(sessionId int64, to Pather) float64
}
...
func Path(sessionId int64, from, to Pather) (path []Pather, distance float64, found bool) {
	nm := nodeMap{}
	nq := &priorityQueue{}
	heap.Init(nq)
	...
	for {
		...
		for _, neighbor := range current.pather.PathNeighbors(sessionId) {
			cost := current.cost + current.pather.PathNeighborCost(sessionId, neighbor)
			...
			if !neighborNode.open && !neighborNode.closed {
				...
				neighborNode.rank = cost + neighbor.PathEstimatedCost(sessionId, to)
